...
const imgDefaultProd = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSk8skqPCAsyApC1tZAVm6jmVb4H5uaCRfd1yfbmjcrEvdsqc8bqYdQkVxT&s=10';
        ...
        async function loadConfig() {
            try {
                ...
                elements.comercioName.innerText = config.comercio || "Pizzaria";
                ...
                section.innerHTML = `
                    ...
                    <div class="products-grid">
                        ${categoriesMap[catName].map(item => {
                            ...
                            const precoItem = Number(String(prices[0].value).replace(',', '.')).toLocaleString('pt-BR', 
                                {
                            ...
                            <div class="product-card">
                                <div class="product-image-container">
                                    ...
                                    <img class="product-img" src="${imgGoogle + item.img}" alt="${item.nome}" onerror="this.src='${imgDefaultProd}'">
                                </div>
                                <div class="product-info">
                                    <h3>${item.nome}</h3>
                                    <p class="product-desc" title="${item.descricao || ''}">${item.descricao || ''}</p>
                                    
                                    <div class="price-container">
                                        ${prices.map(p => `
                                            <div class="price-item">
                                                <div class="price-value">R$ ${precoItem}</div>
                                                <div class="price-label">${p.label}</div>
                                                <button class="add-to-cart-btn" onclick="addToCart('${item.nome}','${item.categoria}', '${p.value}', '${p.label}')">+ Adicionar</button>
                                            </div>
                                        `).join('')}
                                    </div>
                                </div>
